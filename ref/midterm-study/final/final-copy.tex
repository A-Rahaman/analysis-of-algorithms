...
Thus, the probability of failure for a single client is at most $n^{-4}$ and the probability of failure for any client in this way by union bounds is $n^{-3}$.  
Hence, the probability of failure for any client in any way, is at most $n^{-1} + n^{-3} = O(n^{-1})$}
