...
constructing a network where the source is connected to all the
pixels with capacity $f_i$ and the sink is connected by all the pixels with
capacity $b_i$
Two edges $i,j$ and $j,i$
with $p_{ij}$ capacity are added between two adjacent
pixels. The s-t cut-set then represents the pixels assigned to the foreground
in $A$ and pixels assigned to background in $B$.
